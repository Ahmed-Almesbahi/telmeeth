...
function isSuccess(response: any) {
  if (response.status && response.status === false && response.message) {
 ...
 * @returns {any}
 */
async function _post_multipart(url: string, data: any, token = true) {
  // console.log('_post_multipart data', data);
  if (Platform.OS === 'web') {
    try {
      const instance = getAxiosInstance(token, {
 ...
 * @returns {any}
 */
async function _put(url: string, params = {}, token = true) {
  try {
    const instance = getAxiosInstance(token);
 ...
 * @returns {any}
 */
async function _delete(url: string, params = {}, token = true) {
  try {
    const instance = getAxiosInstance(token);
  ...
  const instance = axios.create({
    ...
    headers: token
      ? {
          Authorization: 'Bearer ' + store.getState().user.access_token,
          lang:
            store.getState().language.locale === 'ar'
